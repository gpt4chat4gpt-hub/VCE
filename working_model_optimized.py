...
tau = 0.1            # Relaxation parameter for phi-equation keep it .01
gamma = 10.0             # Relaxation parameter for w-equation
c1 = 0.5                 # Flory–Huggins convex coefficient
c2 = 1            # Concave (quadratic) coefficient  as 1 
...
def run_main_simulation(store_history = False):
    """
    ...
    store_history : bool, optional
        If True, the full space–time history of φ is stored and returned.  When
        False, the solver behaves as originally written (no return).

    Returns
    -------
    phi_hist : ndarray, shape (M+1, N+1)
        The full field history of φ over time (only when ``store_history`` is True).
    x : ndarray, shape (N+1,)
        Spatial grid used in the simulation (only when ``store_history`` is True).
    t_hist : ndarray, shape (M+1,)
        Time stamps corresponding to each stored state in ``phi_hist`` (only when
        ``store_history`` is True).

    Notes
    -----
    The forward solver uses a fixed time step ``dt``.  The number of time
    intervals is ``int(np.ceil(T / dt))``, and the history arrays are sized
    accordingly.  If ``store_history`` is False, the solver runs as
    originally designed and returns nothing.
    """
    ...
    while current_time < T - time_tol:
        ...
        # w^{n+1}
        u_n   = zero_source  # Avoid function call if source is zero
        u_np1 = zero_source
        ...
        if store_history:
            ...
            if n_store < phi_hist.shape[0]:
                ...
                # In the unlikely event that the number of steps exceeds
                # preallocation, resize the history arrays. This should not
                # happen for fixed dt.
                phi_hist = np.vstack([phi_hist, phi[None, :]])
                t_hist = np.append(t_hist, min(current_time, T))
                n_store += 1

        if step % 100 == 0 or current_time >= T:
